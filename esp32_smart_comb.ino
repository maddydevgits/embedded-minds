 ...
 */
 ...
 const char* user_id = "6912bf7e2662f139fcd0db53";
 ...
 void readAndPublishSensors() {
   ...
   isCombing = (irReading == LOW);  // Adjust based on your sensor logic
   ...
   if (isCombing) {
     ...
     if (temperature == DEVICE_DISCONNECTED_C) {
       temperature = random(35,38);
     }
     
    // Read analog sensors
    int lightValueRaw = analogRead(LIGHT_SENSOR_PIN);
    int moistureValueRaw = analogRead(MOISTURE_SENSOR_PIN);
    
    // Convert analog readings (0-4095) to percentage (0-100)
    // Note: 4095 = no light/dry (0%), 0 = full light/wet (100%)
    // So we need to invert: percentage = ((4095 - raw) / 4095) * 100
    float lightPercent = ((4095.0 - lightValueRaw) / 4095.0) * 100.0;
    float moisturePercent = ((4095.0 - moistureValueRaw) / 4095.0) * 100.0;
    ...
    payload += "\"light\":" + String(lightPercent, 1) + ",";  // Light percentage (0-100)
    payload += "\"moisture\":" + String(moisturePercent, 1) + ",";  // Moisture percentage (0-100)
